...
export class DashboardService {
    ...
    async getStats() {
        const equipamentos = await this.equipmentRepo.find({ where: { ativo: true } });
        const totalEquipamentos = equipamentos.length;
        const emUso = equipamentos.reduce(
            (acc, eq) => acc + (eq.quantidadeTotal - eq.quantidadeDisponivel),
            0,
        );
        ...
        for (const cl of checklists) {
            checklistsPorStatus[cl.status] = (checklistsPorStatus[cl.status] ?? 0) + 1;
        }
